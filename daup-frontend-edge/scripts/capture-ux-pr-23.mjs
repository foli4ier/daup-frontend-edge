/**
 * Capture real Hub You. stills for docs/ux/pr-23.
...
const BUST = `ux-you-doors-${Date.now()}`;
...
try {
  ...
  await waitFor(page, '[data-testid="hub-settings"]');
  await page.waitForFunction(() => {
    const settings = document.querySelector('[data-testid="hub-settings"]');
    const text = settings?.textContent || '';
    return Boolean(
      text.includes('Register a new house.')
      && text.includes('Delete the house.')
      && text.includes('Ask for an enhancement.')
    );
  });
